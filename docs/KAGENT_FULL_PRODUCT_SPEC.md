...
## 42. Программная среда агента

### 42.1. Принцип

Модель получает не каталог инструментов, а одну программную среду. Чтение и правка файлов, запуск команд проекта, вызов навыков и порождение субагентов выполняются кодом внутри живого ядра, а не отдельными вызовами инструментов.

Причина: каждый инструмент стоит места в контексте, а каждый промежуточный результат при раздельных инструментах проходит через модель. Композиция внутри ядра устраняет и то, и другое.

### 42.2. Единственный инструмент модели

- базовый runtime предоставляет модели один инструмент — исполнение в постоянном ядре;
- возможности выражаются типизированными вызовами внутри ядра, а не определениями инструментов;
- расширения могут регистрировать дополнительные инструменты осознанно; базовый runtime не требует отдельного инструмента на каждую возможность.

### 42.3. Состояние ядра

- переменные, импорты, функции, разобранные данные и handle субагентов сохраняются между шагами;
- состояние переживает компактификацию и перезапуск ядра через ссылку на снимок;
- объявляются предельный размер и срок жизни состояния;
- состояние доступно для инспекции из Run Timeline. Состояние, которое переживает компактификацию и которое никто не может посмотреть, не допускается;
- состояние ядра входит в сверку при восстановлении после сбоя.

### 42.4. Typed Host Bridge

Ядро не владеет авторитетным состоянием и не хранит credentials. Только через типизированные host-запросы выполняются:

- вызовы провайдеров моделей;
- доступ к credentials и выдача секретов;
- планирование;
- создание субагентов;
- approvals;
- запись в audit;
...
Хост обязан проверить schema, capability grants и политику до выполнения.
...
```json
{
  ...
  "instructions": "Проверь авторизацию",
  "budget": {
    "max_tokens": 50000,
    "max_cost_usd": 3
  }
...
Библиотека внутри ядра является тонким шимом. Она не вызывает провайдеров моделей, не реализует agent loop и не владеет персистентностью. Нарушение инварианта возвращает SDK провайдеров в доменную логику агента, что запрещено разделом 7.

### 42.6. Обязательная песочница

Программная среда включается только внутри песочницы:

- rootless container или microVM;
- отдельный UID;
- PID, mount и network namespaces;
- seccomp;
- AppArmor или SELinux при наличии;
- read-only root filesystem;
- временный workspace;
- CPU, RAM, PID и disk limits;
- scoped secrets;
- явные capability grants.

Без песочницы среда не включается ни в одном режиме, включая локальную разработку на одном узле. Изоляция процессов и отдельный UID сами по себе песочницей не считаются.

Shell существует как ограниченная возможность внутри ядра под той же capability-политикой, а не как обходной путь.

---

## 43. Субагенты, сообщения и дерево сессии

### 43.1. Порождение субагента

- вызов возвращает handle сразу после допуска задачи и **никогда не возвращает результат работы ребёнка**;
- handle содержит идентификатор субагента, имя, роль, идентификатор сессии, Worker, статус, модель и время создания;
- ребёнок наследует модель, конфигурацию провайдера, навыки, инструменты и политику повторов родителя, если явно не указано иное;
- результаты приходят только через типизированные сообщения или артефакты.

Запрет на возврат результата обязателен: он устраняет блокирующее ожидание и не позволяет содержимому ребёнка раздувать контекст родителя.

### 43.2. Глубина рекурсии

Раздел 10.2 запрещал субагентам создавать субагентов. Ограничение уточняется: разрешена глубина 2 при выполнении условий:

- бюджет поддерева резервируется у родителя и не может быть превышен;
- Orchestrator сохраняет владение деревом выше второго уровня;
- каждый уровень наследует ограничения инструментов и путей родителя, сузить их можно, расширить нельзя;
- превышение объявленной глубины является ошибкой политики, а не предупреждением.

### 43.3. Реестр субагентов

Реестр, привязанный к родителю, переживает компактификацию, перезапуск ядра, перезапуск Worker и восстановление родительской сессии. Завершённый субагент может оставаться адресуемым, пока открыта родительская сессия. Удаление — вручную, по TTL, по завершении проекта или по лимиту хранения, привязанному к бюджету проекта.

### 43.4. Обмен сообщениями

Типизированная шина сообщений между агентами. Сообщение содержит идентификатор, задачу, отправителя, получателя, тип, версию схемы, время, ограничение размера payload, статус доставки и ссылку на audit.

Направления: родитель → ребёнок, ребёнок → родитель, между соседями, между агентами на разных Worker через Control Plane и брокер. Прямое сетевое соединение между агентами не используется; временный Link создаётся только при реальной необходимости доступа к конкретному сервису.

### 43.5. Сессия как append-only дерево

- сессия является append-only последовательностью типизированных записей, образующих дерево; записи не изменяются и не удаляются;
- типы записей минимум: заголовок сессии, сообщение, смена модели, компактификация, сводка ветки, атрибуция расхода ребёнка, метка, произвольная запись, состояние репозитория, состояние сессии;
- каждая запись содержит идентификатор, идентификатор родителя, время и версию схемы;
- контекст модели строится обходом пути от текущего листа к корню с применением записей компактификации вместо заменённых ими участков;
- авторитетное хранилище — PostgreSQL и object storage; локальный файл транскрипта является кэшем, а не источником истины.

### 43.6. Навигация и форк

- переход к любой записи и продолжение с неё;
- оставленная ветка не удаляется, а суммаризуется, и сводка сама является записью;
- форк создаёт новую сессию от произвольной точки, наследуя путь до неё;
- в сочетании с кассетами (раздел 35) навигация и форк дают контрфактический разбор: повтор от записи N с другой моделью, версией промпта или политикой, без обращения к провайдеру.

### 43.7. Дерево выполнения в интерфейсе

Для каждого узла показываются статус, Worker, модель, токены, стоимость, время, capabilities, дети, артефакты, сообщения, доказательства верификации, число повторов и активная цель.

---

## 44. Долгоживущие сессии

### 44.1. Правило владения

```text
Отключение клиента ≠ остановка задачи
```

Выполнение принадлежит резидентному worker-процессу, а не Web, Desktop, CLI или TUI. При отключении клиента Worker продолжает работу, ядро остаётся активным, субагенты продолжают задачи, события сохраняются, пользователь подключается позже и получает snapshot и повтор событий.

### 44.2. Очередь сообщений и вмешательство

- steering-сообщение доставляется после завершения текущего хода вместе с его вызовами инструментов;
- follow-up доставляется после завершения всей работы;
- прерывание останавливает текущую операцию, не завершая сессию;
- сообщения из очереди можно вернуть в редактор до доставки;
- побочный вопрос выполняется вне контекста сессии и не добавляется в него.

### 44.3. Цели

```yaml
goal_id: goal-124
objective: "Создать полноценную платформу"
status: active
progress:
  - requirements_completed
  - architecture_completed
current_step: implementation
next_step: backend_authentication
success_criteria:
  - tests_pass
...
Цель переживает отключение клиента, компактификацию, перезапуск Worker и перенос задачи.

### 44.4. Heartbeat и расписания

Сессия возвращается к работе после завершения субагента, по интервалу, по расписанию, после появления approval, после восстановления Worker, после появления результата CI и после восстановления внешнего сервиса. Каждый heartbeat и каждое расписание имеют бюджет, условие остановки, предельное число выполнений и связанную цель.

### 44.5. Ограниченный автономный режим
...
Остановка обязательна при выполнении критериев успеха, исчерпании бюджета, необходимости approval, повторяющейся ошибке, отсутствии прогресса, нарушении политики безопасности, потере Worker и отмене пользователем.

**Достижение лимита не является успешным выполнением.** Задача, остановленная по лимиту, получает статус незавершённой и не засчитывается ни в одну метрику успеха.

### 44.6. Восстановление после сбоя

1. определить последнее поколение;
2. проверить lease;
3. восстановить snapshot;
4. восстановить реестр детей;
5. проверить состояние ядра;
6. повторить события;
7. отметить операции с неизвестным исходом;
8. выполнить сверку через ledger внешних эффектов (раздел 37.4);
9. продолжить либо запросить решение человека.

Шаги 7 и 8 используют состояние `unknown` из ledger эффектов и не являются отдельным механизмом.

---

## 45. Harness, навыки и расширения
...
Разрешено улучшать: дополнительные промпты, память, описания навыков, спецификации субагентов, подсказки workflow, соглашения проекта.

Запрещено изменять автоматически: базовую политику безопасности, авторизацию, политику секретов, правила approval, требования аудита, ограничения песочницы, неизменяемый системный промпт.

Harness Registry и реестр промптов (раздел 35.4) являются одним компонентом, а не двумя.

### 45.2. Refinement на доказательствах

1. проанализировать траекторию;
2. найти конкретную повторяющуюся проблему;
3. предложить небольшое изменение;
4. показать доказательства;
5. показать diff;
6. запустить eval-suite платформы (раздел 41.1);
7. создать snapshot;
8. применить версионированное изменение;
9. поддержать откат.

Каждое изменение имеет область действия, исходную задачу, автора, доказательства, статус оценки и срок действия. Без eval-suite refinement не запускается.

### 45.3. Snapshots и откат

История изменений, diff, снимки, откат, закрепление версии, срок действия, область проекта/личная/организации, approval и автоматический откат при ухудшении метрик качества.

### 45.4. Навыки как пакеты
...
Навык с программной частью является надмножеством инструкционного: он даёт и руководство, и типизированный API, и схемы, и зависимости, и тесты. Навык может сам обращаться к субагентам в пределах разрешённой глубины.

### 45.5. Ленивая загрузка

В стартовый промпт помещается только метаданные навыка: имя, описание, версия, capabilities, уровень риска. Полный текст и API загружаются при совпадении задачи.

### 45.6. Уровни доверия

built-in, verified, organization-approved, project-local, third-party, untrusted.

Для каждого уровня задаются профиль песочницы, доступ к сети, область секретов, capabilities и политика approval. Автоматическая установка непроверенных навыков и расширений запрещена.

### 45.7. Поверхность расширений

Расширение получает события жизненного цикла, ресурсов, сессии, агента, модели, инструментов, пользовательских команд и ввода, и может:

- регистрировать инструменты, команды, горячие клавиши и отображение сообщений;
- добавлять типизированные записи в сессию;
- запрашивать использование контекста и вызывать компактификацию;
- создавать сессию, форкать от записи, перемещаться по дереву;
- отправлять сообщения от своего имени.

Расширения подчиняются уровням доверия из 45.6 и не могут изменять неизменяемый слой harness.

---

## 46. Приватность маршрутизации

### 46.1. Ограничения как часть решения о маршрутизации

Классификация данных не является точкой контроля. Ограничение приватности выражается как жёсткое условие маршрутизации и проверяется до отправки запроса.

Для каждого класса приватности объявляются:

- требование zero data retention;
- запрет на обучение и сбор данных на стороне поставщика;
- разрешённый набор поставщиков;
- исключённый набор поставщиков;
- допустимость fallback;
- требуемый регион размещения.

```yaml
privacy_class: source_code
require_zero_retention: true
refuse_data_collection: true
allow_providers: [anthropic, local]
deny_providers: [aggregators]
fallback_allowed: true
residency: eu
```
...
- каждый адаптер поставщика объявляет retention, обучение, регион и сертификации структурно; необъявленное свойство считается неудовлетворённым;
- конфигурация модели, не удовлетворяющая ограничениям, не является кандидатом;
- при отсутствии кандидатов шаг завершается ошибкой политики и порождает запрос решения; тихая деградация к менее защищённому поставщику запрещена;
- кандидаты fallback наследуют ограничения исходного запроса;
- удовлетворённый набор ограничений записывается вместе с решением о маршрутизации в Run Timeline и audit log;
- для классов, помеченных как локальные, рассматриваются только локальные конфигурации; флага обхода не существует, изменение класса является аудируемым действием;
- оптимизация стоимости работает строго внутри допустимого по приватности множества.

---

## 47. Версионирование протоколов

### 47.1. Классификация изменений

Каждое изменение команды, события или формы ответа на границе классифицируется до реализации:

- **обратно совместимое** — аддитивное, игнорируемое старым участником; версия не поднимается;
- **за capability** — новое поведение доступно только когда обе стороны объявили возможность; версия не поднимается, имя capability становится частью контракта;
- **несовместимое** — старый участник не может работать корректно; поднимается версия протокола и описывается путь миграции.

### 47.2. Согласование

При соединении стороны обмениваются версией протокола и набором capabilities. Клиент обязан проверить наличие возможности до её использования, сервер не предполагает возможность, о которой не сообщили. Неизвестные поля игнорируются, неизвестные capabilities не предполагаются.

### 47.3. Доказательства совместимости

Каждое изменение формата обновляет карту совместимости и добавляет тесты в обе стороны: новый клиент против старого сервера и старый клиент против нового сервера. Изменение формата без обоих тестов не принимается.

### 47.4. Окно поддержки

Объявляется и принудительно применяется диапазон поддерживаемых версий. Участник ниже нижней границы получает явную ошибку с описанием действия, а не непонятный отказ. Для зарегистрированного Worker такой отказ является инцидентом.

Область применения: Worker API, протокол подключения клиентов, инфраструктурный мост, webhooks и конверт событий брокера.

---

## 48. Распределённое исполнение

Раздел уточняет и заменяет части раздела 16.

### 48.1. Worker

Worker — удалённый исполнитель без собственного Control Plane. Типы: general, coding, test, research, browser, GPU, model, subagent runtime.

Worker выполняет только выданные capabilities и работает внутри песочницы раздела 42.6.

### 48.2. Enrollment

1. Control Plane создаёт одноразовый token со сроком не более 10 минут;
2. token связан с идентификатором узла и типом Worker;
3. Worker локально создаёт ключ и CSR;
4. Gateway проверяет token и выдаёт сертификат;
5. token уничтожается.

Общий постоянный token и передача приватного ключа запрещены.
...
```json
{
  ...
  "allowed_tools": ["git.read", "code.edit", "test.run"],
...
При потере связи Worker может завершить уже подтверждённую задачу до истечения lease, но не может получать новые задачи, расширять права, создавать Links и продлевать lease самостоятельно.

Результат работы, выполненной без связи, попадает в систему только через ledger внешних эффектов (раздел 37.4): повторная выдача той же задачи другому Worker обязана обнаружить существующий эффект по ключу идемпотентности, а не выполнить его второй раз.

### 48.4. Scheduler

Учитывает доступность, CPU, RAM, disk, GPU, задержку и длину очереди, capabilities, регион, класс приватности, стоимость, режим обслуживания, состояние безопасности, affinity и anti-affinity.

Режимы: Manual, Balanced, Cheapest, Fastest, Local-only, Private-node-only, GPU-required, Isolated, Affinity, Anti-affinity.

### 48.5. Пулы агентов

```yaml
name: coding-pool
members: [worker-main, worker-build-01, worker-build-02]
strategy: least_loaded
max_parallel_tasks: 12
```

Пулы: coding, test, research, GPU, browser, private, low-cost, high-security.

### 48.6. Передача результатов

Результаты передаются через неизменяемые артефакты и object storage, а не прямой передачей рабочих каталогов. Артефакт имеет адресацию по содержимому, контрольную сумму, шифрование, срок хранения, область проекта, проверку на вредоносное содержимое, ограничение размера, подписанный манифест, audit и возобновляемую загрузку.

### 48.7. Worker API

```text
POST /api/v1/workers/enroll
POST /api/v1/workers/renew
POST /api/v1/workers/heartbeat
GET  /api/v1/workers/tasks/next
POST /api/v1/workers/tasks/{id}/accept
POST /api/v1/workers/tasks/{id}/progress
POST /api/v1/workers/tasks/{id}/complete
POST /api/v1/workers/tasks/{id}/fail
POST /api/v1/workers/artifacts/init
POST /api/v1/workers/artifacts/complete
POST /api/v1/workers/leases/{id}/renew
POST /api/v1/workers/leases/{id}/release
```

Протокол подчиняется правилам раздела 47.

### 48.8. Безопасность субагентов

Временная идентичность связывается с задачей, проектом, Worker, инструментами, репозиториями, узлами, TTL и лимитами расхода. После завершения идентичность отзывается, секреты очищаются, workspace уничтожается, Link отключается, lease закрывается, audit сохраняется.

---

## 49. Интеграция с инфраструктурным слоем

### 49.1. Принцип

KAgent и Server Monitor Manager остаются разными продуктами. KAgent работает без SMM; при подключении SMM появляется инфраструктурный функционал.

KAgent получает: серверы, метрики, alerts, сервисы, контейнеры, Links, provisioning jobs, backups, drift, capacity, состояние безопасности.

KAgent может запросить: диагностику, временный Link, установку, обновление и удаление Worker, план provisioning, backup, перезапуск сервиса, remediation.

KAgent не может получить: root shell, приватные ключи VPN, управление CA, отключение audit, произвольное выполнение команд.

### 49.2. Мост

```text
services/infrastructure-bridge/
packages/smm-contracts/
```

Мост отвечает за обнаружение, регистрацию, согласование capabilities, события, узлы, метрики, инфраструктурные задания и передачу capacity планировщику.
...
### 49.3. Идентичность интеграции

Отдельная роль интеграции, связанная с идентификатором установки, организацией, узлами, capabilities, сроком действия и fingerprint. Реализуется как частный случай automation identity инфраструктурного слоя, а не как новая сущность на его стороне.
...
infrastructure.services.read
infrastructure.containers.read
infrastructure.logs.read
infrastructure.backups.read
...
Запрещены по умолчанию: firewall.apply, users.modify, ca.rotate, node.delete, secrets.read, root.execute.

Набор capabilities объявляется динамически; интерфейс показывает только реально доступные функции.

### 49.5. Установка Worker

```text
запрос KAgent → неизменяемый план SMM → подтверждение оператора
→ установка → локальная генерация ключа → enrollment → аттестация → пул планировщика
```

### 49.6. Временные Links

Link связан с задачей, TTL, источником, назначением, протоколом, портом, причиной и версией политики. Отключается автоматически после завершения, ошибки, timeout или истечения lease.

### 49.7. Отказы

При недоступности инфраструктурного слоя KAgent продолжает работу, не создаёт новые инфраструктурные задания и показывает мониторинг как устаревший. При недоступности Worker планировщик прекращает выдачу задач, ждёт истечения lease и переносит задачу только при безопасном повторе.

### 49.8. Условие реализуемости

Разделы 49.1–49.7 требуют парной работы на стороне инфраструктурного слоя: файла обнаружения, сокета, роли интеграции и типизированного действия установки Worker. Ни один из этих элементов там сейчас не существует. Пункты интеграции не ставятся в работу до появления соответствующих пунктов в плане инфраструктурного слоя и до физической приёмки управляемых Links.

---

## 50. Операционные требования

### 50.1. Установка

Установщик распространяется как версионированный релиз с публикуемой контрольной суммой и подписью. Установщик проверяет контрольную сумму скачанного релиза до выполнения. Схема «скачать скрипт и передать его в интерпретатор с правами суперпользователя» без проверки подписи и контрольной суммы запрещена, включая примеры в документации.

Установщик проверяет ОС, устанавливает runtime, создаёт секреты, настраивает домен и HTTPS, применяет миграции, создаёт владельца, выполняет проверку работоспособности и готовит backup, обновление, откат и удаление.

### 50.2. Команды диагностики

```text
kagent status
kagent agents
kagent workers
kagent attach <session>
kagent resume <session>
kagent doctor
kagent doctor --fix
kagent update
kagent shutdown
...
`doctor` проверяет Gateway, Control Plane, брокер, PostgreSQL, object storage, Workers, ядра, песочницу, поставщиков моделей, инфраструктурный мост, сертификаты, диск и просроченные lease.

### 50.3. Головные режимы

CLI, print, JSON, потоковый JSON, локальный и удалённый RPC, SDK. Это делает KAgent вызываемым из CI, IDE, других программ, автоматизаций и инфраструктурного слоя.

### 50.4. Порождённые процессы

Каждый процесс, порождённый агентом, записывается в журнал с идентификатором процесса, идентификатором владельца и идентификатором запуска процесса, защищающим от переиспользования идентификаторов. Журнал записывается на диск синхронно. При старте Worker подбирает и завершает процессы, оставшиеся от предыдущего поколения. Утечка процессов после падения Worker является дефектом, а не особенностью среды.

### 50.5. Наблюдаемость расхода

Интерфейс показывает расход по родительской сессии и по каждому субагенту отдельно: токены, стоимость, использование контекста. Попадания в кэш отображаются явно, чтобы запуск не выглядел дешевле, чем он есть.

---

## 51. Согласование словаря и статус решений

### 51.1. Приоритет документов

Настоящее ТЗ является источником требований. Внешние документы с предложениями вносятся в него разделами и после внесения самостоятельной силы не имеют.

### 51.2. Разрешённые расхождения

| Предмет | Решение |
|---|---|
| Роли пользователей | Сохраняются восемь ролей раздела 4. Automation, Worker и Satellite являются сервисными идентичностями, а не пользовательскими ролями, и описываются в разделе 4.8 |
| Роль менеджера проекта | Сохраняется |
| Personal Assistant Agent | **Сохраняется.** Персональный помощник остаётся целью продукта, изоляция личного контура описана в разделе 40.2 |
| Specification Verifier и Documentation Agent | Сохраняются в списке ролей раздела 10.1 |
| Repair Agent и Infrastructure Agent | Добавляются в раздел 10.1 |
| Типы узлов | Используется таксономия раздела 48.1; таксономия раздела 16.1 заменяется ею |
| Рекурсия субагентов | Ограничение раздела 10.2 смягчается до глубины 2 согласно разделу 43.2 |
| Quality gates | Являются оболочкой; содержание задаёт раздел 36 о целостности верификации |
| Harness Registry | Совпадает с реестром промптов раздела 35.4 |
| Сверка после сбоя | Совпадает с ledger внешних эффектов раздела 37.4 |

### 51.3. Отложенные решения

**Satellite и федерация выносятся в отдельное техническое задание.** Автономная установка с собственным Control Plane, собственными проектами и приёмом делегированных задач означает два авторитетных источника истины. Модель владения, разрешение конфликтов при восстановлении связи и правила слияния состояния не проработаны и не могут быть проработаны как пункт списка реализации.

До появления отдельного ТЗ Satellite не входит в план и не учитывается в оценках сроков.

### 51.4. Заимствование

Идеи разделов 42–45 и 50 частично основаны на публично доступных архитектурных решениях сторонних агентских платформ. Заимствуются архитектурные решения, а не исходный код: правило раздела о независимости от стороннего кода сохраняет силу. Вендоринг стороннего кода под совместимой лицензией допускается только отдельным решением владельца проекта с сохранением уведомлений об авторстве.
